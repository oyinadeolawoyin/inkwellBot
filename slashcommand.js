...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("project")
        .setDescription("What are you working on? (optional, e.g. 'Chapter 3 of my novel')")
        .setRequired(false)
        .setMaxLength(100)
    ),
...
async function handleMyId(interaction) {
  try {
    // Send a DM directly to the user — fully private, nobody else sees it
    await interaction.user.send(
      `🔐 Your Discord ID is: \`${interaction.user.id}\`\n\n` +
      `To link your Discord account with Inkwell:\n` +
      `1. Copy the ID above\n` +
      `2. Go to your Inkwell settings: https://inkwellinky.vercel.app/settings\n` +
      `3. Paste it in the "Link Discord" field and save\n\n` +
      `Once linked, you can start and join sprints directly from Discord 🌱`
    );

    // Confirm in the channel that the DM was sent — ephemeral so only they see it
    await interaction.reply({
      content: "📬 I've sent your Discord ID to your DMs — check your messages!",
      ephemeral: true,
    });
  } catch (err) {
    ...
    await interaction.reply({
      content:
        ...
        `https://inkwellinky.vercel.app/settings\n\n` +
        `*(Couldn't send a DM — make sure your DMs are open if you'd prefer that)*`,
...
async function handleJoin(interaction) {
  ...
  try {
    ...
    const options = soundscapes.slice(0, 24).map((s) => ({
      ...
      value: `${s.id}::${activeSprint.id}::${startWords}::${encodedProject}`,
    }));

    options.unshift({
      label: "No soundscape",
      description: "Write in silence",
      value: `none::${activeSprint.id}::${startWords}::${encodedProject}`,
    });
...
async function handleSoundscapePick(interaction) {
  ...
  const [soundscapeRaw, groupSprintId, startWordsRaw, projectRaw] = interaction.values[0].split("::");
  const soundscapeId = soundscapeRaw === "none" ? null : Number(soundscapeRaw);
  const startWords = Number(startWordsRaw) || 0;
  const project = projectRaw ? decodeURIComponent(projectRaw) : "";
  ...
  try {
    ...
    await interaction.channel.send(
      `🖊️ **${user.username}** just joined the sprint${wordNote}${soundNote} — sending you positive vibes! 🌱${projectNote}\n\n` +
      `Come write together:\n${link}`
    );
  } catch (err) {
    if (err.message === "LINK_REQUIRED") {
